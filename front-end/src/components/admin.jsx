    ...
    const getUsers = async () => {
      try {
        ...
        let approved = await axios(
          `${backendApiUrl}/users/authorized/${currentUser.uid}`
        );
        setUsers(approved.data.filter((x) => x.role != "Admin"));

        let pending = await axios(
          `${backendApiUrl}/users/pending/${currentUser.uid}`
        );
  ...
  const removeUser = async (user) => {
    ...
    try {
      if (pendingUsers.map((x) => x.email).includes(email)) {
        deleteUserFirebaseId = pendingUsers.filter((x) => x.email == email)[0]
          .firebaseId;
        let v = [...pendingUsers];
        v = v.filter((x) => x.email != email);
        setPendingUsers(v);
      } else if (users.map((x) => x.email).includes(email)) {
        deleteUserFirebaseId = users.filter((x) => x.email == email)[0]
          .firebaseId;
        let v = [...users];
        v = v.filter((x) => x.email != email);
        setUsers(v);
    ...
    <>
      ...
      <div>
        ...
        <ul>
          {users.map((x) => (
            <li key={x.name}>
              {x.name}-{x.email}-{x.employeeId}{" "}
              <button
                style={{
                  backgroundColor: "rgb(191, 34, 34)",
                  color: "rgb(255, 255, 255)",
                }}
                value={x.email}
                onClick={(y) => removeUser(y)}
              >
                Remove
              </button>
            </li>
        ...
        <ul>
          {pendingUsers.map((x) => (
            <li key={x.name}>
              ...
              <button
                style={{ backgroundColor: "rgb(10, 209, 97)" }}
                value={x.email}
                onClick={(y) => allowUser(y)}
              >
                Accept
              </button>
            </li>
